...
self.addEventListener('fetch', (event) => {
  ...
  // Run migration 0015 (dataPending + unique index on places)
  app.get("/api/run-migration-0015", async (req, res) => {
    const { key } = req.query;
    ...
    try {
      ...
      const migrationPath = path.join(process.cwd(), "drizzle", "0015_add_place_data_pending.sql");
      ...
      for (const statement of statements) {
        try {
          await db.execute(statement);
          results.push(`OK: ${statement.substring(0, 60)}...`);
        } catch (err: any) {
          ...
          if (
            ...
            results.push(`SKIP (already exists): ${statement.substring(0, 60)}...`);
          } else {
            throw err;
          }
        }
      ...
      return res.json({
        success: true,
        message: "Migration 0015 (dataPending + places_name_city_idx) executed",
        results,
      });
    } catch (error: any) {
      console.error("Migration 0015 error:", error);
      return res.status(500).json({
        error: 'Migration failed',
        details: error.message,
      });
